...
const url = encodeURIComponent(campaign.shareUrl)
const text = encodeURIComponent(campaign.shareText)
...
const channels = [
  ...
  },
  {
    ...
    color: 'bg-[#1877F2] hover:bg-[#0f63d6]',
    icon: FacebookIcon,
  },
  {
    label: 'X',
    href: `https://twitter.com/intent/tweet?text=${text}&url=${url}`,
    color: 'bg-foreground hover:opacity-90',
    icon: XIcon,
  },
  {
    label: 'LinkedIn',
    href: `https://www.linkedin.com/sharing/share-offsite/?url=${url}`,
    color: 'bg-[#0A66C2] hover:bg-[#08529e]',
    icon: LinkedInIcon,
  },
  {
    label: 'Telegram',
    href: `https://t.me/share/url?url=${url}&text=${text}`,
    color: 'bg-[#26A5E4] hover:bg-[#1d8fc7]',
    icon: TelegramIcon,
  },
  {
    label: 'Email',
    href: `mailto:?subject=${title}&body=${text}%20${url}`,
    color: 'bg-teal hover:opacity-90',
    ...
    <div className={cn('w-full', className)}>
        ...
        {channels.map((c) => {
          ...
          return (
            <a
              ...
              className={cn(
                'flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold text-white shadow-sm transition-transform hover:-translate-y-0.5',
                c.color,
              )}
            >
              <Icon />
      ...
      <div className="mt-4 flex flex-col gap-3 sm:flex-row">
        ...
        >
          {copied ? (
            <>
              <Check className="size-5 text-success" /> Link Copied!
            </>
          ) : (
            <>
        ...
        >
          <Share2 className="size-5" /> Share Appeal
        </button>
      </div>
    </div>
  )
